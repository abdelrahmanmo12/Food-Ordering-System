...
// ─── Hooks ─────────────────────────────────────────────────────────────────
import { useApproveOwner, useRejectOwner, useOwnerRequests } from '../hooks/users';
...
function usePendingRestaurants() {
  return useQuery({
    queryKey: ["pending-restaurants"],
    queryFn: () => api.get("/restaurants/admin/requests"),
  });
}
...
function AdminDashboard() {
  ...
  const [accountStatuses, setAccountStatuses] = useState({}); // { profileId: status }
  ...
  const { data: pendingRestaurants = [], isLoading: loadingPR } = usePendingRestaurants();
  ...
  const approveOwnerMutation = useApproveOwner();
  const rejectOwnerMutation = useRejectOwner();
  ...
  const updateAccountStatus = useMutation({
    ...
    onSuccess: (_, { id, status }) => {
      ...
      setAccountStatuses(prev => ({ ...prev, [id]: status }));
    },
  });
  ...
  const updateRestaurantStatusMutation = useMutation({
    mutationFn: ({ id, status }) => api.patch(`/restaurants/admin/requests/${id}/status`, null, {
      query: { status }
    }),
    onSuccess: () => {
      queryClient.invalidateQueries(["pending-restaurants"]);
      queryClient.invalidateQueries(["restaurants"]);
    },
  });
  ...
  const stats = [
    ...
    { label: "Active Now", value: restaurants.filter(r => (r.isOpen ?? r.opened) && r.status === "APPROVED").length, icon: "🟢", color: "#5b8dd9" },
    { label: "Pending Owners", value: pendingOwners.length, icon: "⏳", color: "#f5a623" },
    { label: "Pending Resto", value: pendingRestaurants.length, icon: "🏪", color: "#f5a623" },
  ];

  const tabs = [
    { key: "restaurants", label: "🏪 Restaurants" },
    { key: "restaurantRequests", label: `🏪 Resto Requests${pendingRestaurants.length ? ` (${pendingRestaurants.length})` : ""}` },
    { key: "ownerRequests", label: `🧑‍🍳 Owner Requests${pendingOwners.length ? ` (${pendingOwners.length})` : ""}` },
    { key: "profiles", label: "👥 User Profiles" },
    { key: "accounts", label: `🔐 Pending Accounts${pendingAccounts.length ? ` (${pendingAccounts.length})` : ""}` },
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px" }}>
              ...
              <tbody>
                {errorR ? (
                  ...
                  <tr key={r.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    ...
                    <td style={{ padding: "14px 20px" }}>{(r.isOpen ?? r.opened) ? "Open" : "Closed"}</td>
      ...
      {}
      {tab === "restaurantRequests" && (
        <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
          <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border)" }}>
            <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20 }}>Restaurant Registration Requests</h3>
            <p style={{ color: "var(--text2)", fontSize: 13, marginTop: 4 }}>Approve or reject new restaurant listings.</p>
          </div>
          {loadingPR ? (
            <div style={{ padding: 40, textAlign: "center" }}>Loading requests...</div>
          ) : pendingRestaurants.length === 0 ? (
            <div style={{ padding: 40, textAlign: "center", color: "var(--text2)" }}>✅ No pending restaurant requests.</div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "var(--bg3)", borderBottom: "1px solid var(--border)" }}>
                    {["ID", "Name", "Cuisine", "Location", "Actions"].map(h => (
                ...
                <tbody>
                  {pendingRestaurants.map((r, i) => (
                    <tr key={r.id} style={{ borderBottom: "1px solid var(--border)", animation: `fadeUp 0.3s ${i * 0.04}s ease both` }}>
                      <td style={{ padding: "14px 20px", color: "var(--text3)", fontSize: 13 }}>#{r.id}</td>
                      <td style={{ padding: "14px 20px", fontWeight: 600 }}>{r.name}</td>
                      <td style={{ padding: "14px 20px" }}>{r.cuisine || "N/A"}</td>
                      <td style={{ padding: "14px 20px", fontSize: 13 }}>{r.location || "N/A"}</td>
                      <td style={{ padding: "14px 20px" }}>
                        <div style={{ display: "flex", gap: 8 }}>
                          <Btn 
                            variant="danger" 
                            ...
                            onClick={() => updateRestaurantStatusMutation.mutate({ id: r.id, status: "BANNED" })}
                          >
                            Reject
                          </Btn>
                          <Btn 
                            size="sm"
                            disabled={updateRestaurantStatusMutation.isPending}
                            onClick={() => updateRestaurantStatusMutation.mutate({ id: r.id, status: "APPROVED" })}
                          >
                            ✓ Approve
                          </Btn>
                        </div>
      ...
      {tab === "profiles" && (
        <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
          <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border)" }}>
            <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20 }}>User Profiles</h3>
            <p style={{ color: "var(--text2)", fontSize: 13, marginTop: 4 }}>Manage system users and their account status.</p>
          </div>
          {loadingProfiles ? (
            <div style={{ padding: 40, textAlign: "center" }}>Loading profiles...</div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "var(--bg3)", borderBottom: "1px solid var(--border)" }}>
                    {["ID", "Name", "Type", "Phone", "Actions"].map(h => (
                      <th key={h} style={{ padding: "12px 20px", textAlign: "left", fontSize: 12, color: "var(--text3)", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {profiles.map((p, i) => (
                    <tr key={p.profileId} style={{ borderBottom: "1px solid var(--border)", animation: `fadeUp 0.3s ${i * 0.04}s ease both` }}>
                      <td style={{ padding: "14px 20px", color: "var(--text3)", fontSize: 13 }}>#{p.profileId}</td>
                      <td style={{ padding: "14px 20px", fontWeight: 600 }}>{p.fullName}</td>
                      <td style={{ padding: "14px 20px" }}>
                        <Badge color={p.type === "ADMIN" ? "var(--green)" : p.type === "OWNER" ? "var(--amber)" : "#5b8dd9"}>{p.type}</Badge>
                      </td>
                      <td style={{ padding: "14px 20px", fontSize: 13 }}>{p.phone || "N/A"}</td>
                      <td style={{ padding: "14px 20px" }}>
                        <div style={{ display: "flex", gap: 8 }}>
                          <Btn 
                            variant="danger" 
                            size="sm"
                            disabled={updateAccountStatus.isPending || p.type === "ADMIN" || accountStatuses[p.profileId] === "BANNED"}
                            onClick={() => updateAccountStatus.mutate({ id: p.profileId, status: "BANNED" })}
                            title={p.type === "ADMIN" ? "Cannot ban an administrator" : accountStatuses[p.profileId] === "BANNED" ? "Already banned" : ""}
                          >
                            {accountStatuses[p.profileId] === "BANNED" ? "Banned" : "Ban"}
                          </Btn>
                          <Btn 
                            size="sm"
                            variant="ghost"
                            disabled={updateAccountStatus.isPending || p.type === "ADMIN" || accountStatuses[p.profileId] === "ACTIVE"}
                            onClick={() => updateAccountStatus.mutate({ id: p.profileId, status: "ACTIVE" })}
                            title={p.type === "ADMIN" ? "Cannot modify administrator status" : accountStatuses[p.profileId] === "ACTIVE" ? "Already active" : ""}
                          >
                            {accountStatuses[p.profileId] === "ACTIVE" ? "Active" : "Activate"}
                          </Btn>
                        </div>
                      </td>
                    </tr>
                        ...
                        {req.status === "PENDING" ? (
                          <div style={{ display: "flex", gap: 8 }}>
                            <Btn
                              ...
                              onClick={() => rejectOwnerMutation.mutate(req.accountId)}
                            >
                              Reject
                            </Btn>
                            <Btn
                              ...
                              onClick={() => approveOwnerMutation.mutate(req.accountId)}
                            >
                              ✓ Approve
                            </Btn>
                          </div>
                ...
                <div key={account.id} style={{
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <Btn variant="danger" size="sm" disabled={updateAccountStatus.isPending}
                      onClick={() => updateAccountStatus.mutate({ id: account.accountId, status: "REJECTED" })}>
                      Reject
                    </Btn>
                    <Btn size="sm" disabled={updateAccountStatus.isPending}
                      onClick={() => updateAccountStatus.mutate({ id: account.accountId, status: "ACTIVE" })}>
                      ✓ Approve
                    </Btn>
                  </div>
                </div>
